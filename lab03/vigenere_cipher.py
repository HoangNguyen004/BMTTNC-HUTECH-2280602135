...
class MyApp(QMainWindow):
    ...
    def call_api_encrypt(self):
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                print("Encrypt API response:", data) 
    ...
    def call_api_decrypt(self):
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                print("Decrypt API response:", data)  # Debug thông tin trả về
                if "decrypted_text" in data:
                    self.ui.txt_plain_text.setPlainText(data["decrypted_text"])
                else:
                    self.ui.txt_plain_text.setPlainText("No decrypted_text key in response")
